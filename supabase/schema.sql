...
create or replace function public.create_ghost_profile(p_nom text, p_club_id uuid, p_equipe_id uuid)
returns uuid
language plpgsql
security definer
set search_path = public
...
begin
  ...
  insert into public.profiles (id, role, nom, club_id, equipe_id, is_approved)
  values (v_new_id, 'joueur', p_nom, p_club_id, p_equipe_id, false);
...
using (
  exists (
    ...
    where
      me.role = 'super_admin'
      or (me.role in ('admin', 'parent') and me.club_id is not null and me.club_id = equipes.club_id)
      or (
